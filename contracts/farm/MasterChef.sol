...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        emit NewRewardPerBlock(0, flyPerBlock);
    ...
    function updateFlyPerBlock(uint256 _flyPerBlock) external onlyOwner {
        emit NewRewardPerBlock(flyPerBlock, _flyPerBlock);
        flyPerBlock = _flyPerBlock;
    }
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) external onlyOwner {
        require(address(_lpToken) != address(fly), "can not add fly");
    ...
    function pendingFly(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 flyReward = multiplier.mul(flyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accFlyPerShare = accFlyPerShare.add(flyReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 flyReward = multiplier.mul(flyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        flyReward = fly.mint(flyReward);
        pool.accFlyPerShare = pool.accFlyPerShare.add(flyReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function safeFlyTransfer(address _to, uint256 _amount) internal {
        uint currentBalance = IERC20(fly).balanceOf(address(this));
        if (currentBalance < _amount) {
            _amount = currentBalance;
        }
        IERC20(fly).safeTransfer(_to, _amount);
    }
